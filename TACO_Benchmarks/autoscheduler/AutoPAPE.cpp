...
for (const auto &f_load : group_load_costs) {
  ...
  //  cout<<f_load.first<<std::endl;
   ...
   //   cout<<"============"<<endl;
      ...
      partial_factor+=load_cost*min(1+footprint*load_slope,cost_factor);;;
	      ...
	      cost_factor=cost_factor_shared/(1024);
      load_slope = cost_factor ;
      partial_factor+=load_cost*min(1+footprint*load_slope,cost_factor);;;
      }
  ...
  /*          vector<Expr> gpu_specs=estimate_occupancy(g_analysis.threads_out,shared_mem,estimate_blocks);*/
   ...
   /*     if(!to_inline){
        	if(can_prove(g_analysis.shared_mem>49152))  return GroupAnalysis();
...
bool is_singleton=is_singleton_group(g);
cout<<"is singleton"<<g.members.size()<<" output "<<g.output.stage_num<<endl;
...
for (const FStage &mem : g.members) {
 ...
 cout<<"mem "<<mem.func.name()<<" update "<<mem.stage_num<<endl;
...
vectorize_stage(g, mem_handle, mem.stage_num, mem_def, mem.func,is_output_st,is_singleton,
