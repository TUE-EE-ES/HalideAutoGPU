...
//  disp_pipeline_graph();
    ...
    if(g.inlined.find(st.func.name())!=g.inlined.end())  continue;
    if(st.func.name()==g.output.func.name())  continue;
    if(g.inlined.find(get_expr_str(st.compute_stage))!=g.inlined.end())	st.compute_stage=g.output.func.name();
        int sum=0;
    ...
    for (const FStage &c : get_element(global_children, st)){
      ...
      if(c.func.name()==st.func.name())	continue;
      if(g_members.find(c.func.name())!=g_members.end()&&(g.inlined.find(c.func.name())==g.inlined.end())){
        sum++;
        consumers.push_back(c.func.name());
        }
      else if(g.inlined.find(c.func.name())!=g.inlined.end())	sum++;
    }
    if(sum>1) {
     st.compute_stage=g.output.func.name();
     st.compute_level=Expr();
     } 
    else if ((sum==1))  st.compute_stage=g.output.func.name();
     ...
     //cout<<"Mem "<<st.func.name()<<" compute at "<<st.compute_stage<<" , "<<st.compute_level<<endl;
     }
return g;  
...
if(tile_vars.size()>1){   
      ...
      //  cout<<"GEN TILES "<<tile_vars[index]<<" "<<tiles[index]<<endl;
        ...
        if(can_prove(extents[tile_vars[0]]>1024)&&(can_prove(tiles[index]>=extents[tile_vars[0]]/32))) a[index]=max_int;;
 ...
 if(stg.stage_num>0){
  ...
  if(can_prove(extents[tile_vars[0]]<1024)&&(can_prove(dim_size>extents[tile_vars[0]]/32))) continue;
  }
  map<string, Expr> tiling;
...
if(tile_vars.size()<2){
	for(const auto &ex:extents){
		if(tile_vars.size()>=3)	break;
		if(tile_vars.find(ex.first)!=tile_vars.end())	continue;
		if(can_prove(ex.second>8))	tile_vars.insert(ex.first);
}	
...
Partitioner::find_best_tile_config(const Group &g,bool is_init, bool is_final) {
  ...
  //  cout<<"find best for "<<g.output.func.name()<<" dim size "<<dims.size()<<endl;
    ...
    for(const auto &tv:thread_vars) init_config.emplace(tv,make_one(Int(32)));
...
void Partitioner::generate_group_cpu_schedule(
  ...
  const set<string> &inlines,
  AutoSchedule &sched,bool will_fold) {
  std::string folded_fusion = Internal::get_env_variable("HL_AUTO_FOLDED_FUSION");
  bool  use_folded_fusion_analysis = std::atoi(folded_fusion.c_str());
  Group g=og_group;
  if((use_folded_fusion_analysis)&&(1)) g=optimize_granularity(og_group,sched);
...
if (mem.stage_num > 0){
 ...
 //     if((will_fold)&&(use_folded_fusion_analysis))	Func(mem.func).store_in(MemoryType::Register);
  }
...
//	 for(const auto &gm:g.members)	cout<<gm.func.name()<<endl;
 ...
 Expr thresh=make_const(Float(32),0.3);
    if(can_prove(asda.shared_mem>2*16384)||can_prove(asda.occupancy<thresh)||can_prove(asda.active_threads>900))	will_fold.insert(g.first.func.name());
 // }
	    }
   ...
   /* cout<<"================================="<<endl;
    ...
    for (const auto &g : groups) {
        generate_group_cpu_schedule(g.second, t, get_element(loop_bounds, g.first),
            get_element(storage_bounds, g.first), inlines, sched,will_fold.find(g.first.func.name())!=will_fold.end());
    }
}
...
MachineParams MachineParams::generic() {
    ...
    if (params.empty()) {
        return MachineParams(68, 16 * 1024 * 1024, 4);
    } else {
