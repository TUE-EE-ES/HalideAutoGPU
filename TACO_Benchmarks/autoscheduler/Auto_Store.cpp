...
#include "../../src/AutoScheduleUtils.h"
#include "Errors.h"
...
bool sort_by_Expr(const pair<string,Expr>& a,
                  const pair<string,Expr>& b){
...
//   cout<<"NUM TO ROUND UP "<<simplify(num)<<endl;
    ...
    if(can_prove(fraction>=thresh))  numToRound=simplify(truncated+1);
    else numToRound=simplify(truncated);
    if(can_prove(numToRound==0))  return(make_one(Int(32)));
...
struct Partitioner {
    ...
    struct GroupAnalysis {
        ...
        inline bool defined() const {
          return cost.defined() && parallelism.defined() && threads_out.defined() && n_threads.defined() && occupancy.defined()&&active_threads.defined();
      }
...
pair<map<string,Expr>,vector<pair<FStage,Expr>>> eval_max_threads(const Group &g, bool show_analysis);
...
pair<map<string, Expr>, GroupAnalysis> find_best_tile_config(const Group &g,bool is_init,bool is_final);
...
vector<Expr> estimate_occupancy(const Expr threads, const Expr footprint, Expr n_blocks);
Expr estimate_tile_benefit(const GroupAnalysis &old_grouping, const GroupAnalysis &new_grouping,
...
void vectorize_stage(
  const Group &g, Stage f_handle, int stage_num, Definition def,
  Function func, bool is_group_output,bool is_singleton, const Target &t, set<string> &rvars,
  map<string, Expr> &estimates, AutoSchedule &sched, vector<string> thread_dims);
        //unroll innermostsssss
void unroll_group_outer_stage(
  const Group &g, Stage f_handle, int stage_num, Definition def,
  Function func, bool is_group_output, const Target &t, set<string> &rvars,
  map<string, Expr> &estimates, AutoSchedule &sched, vector<string> thread_dims,vector<string> non_thread_dims);


//unroll / tile + unroll members
void unroll_group_inner_stage(
  const Group &g, Stage f_handle, int stage_num, Definition def,
  Function func, bool is_group_output, const Target &t, set<string> &rvars,
  map<string, Expr> &estimates, AutoSchedule &sched, vector<string> thread_dims,vector<string> non_thread_dims);
...
void reorder_dims(Stage f_handle, int stage_num, Definition def,
  map<string, Expr> strides, AutoSchedule &sched,map<string,Expr> sbounds,vector<string> thread_dims);
...
bool Partitioner::check_for_boundary(const Group &group){
  ...
  const vector<string> bound_conds={"constant_exterior","repeat_edge","repeat_image","mirror_image","mirror_interior"};
  for(const auto &mem:group.members){
    const string &mem_name=mem.func.name();
    if(group.inlined.find(mem_name)!=group.inlined.end())  continue;
    for(const auto &bcs:bound_conds){
      if(mem_name.find(bcs)!=std::string::npos)  return true;
    }
    //if(mem_name.find(
...
bool Partitioner::is_singleton_group(const Group &group){
  ...
  for(const auto &mem:group.members){
    if(group.inlined.find(mem.func.name())!=group.inlined.end())  continue;
    if(mem.func.name()!=group.output.func.name())  return false;
...
Partitioner::Group Partitioner::optimize_granularity(const Group &pre_g,const AutoSchedule &sched){
	...
	Group g=pre_g;
      ...
      //compute all at their consumers IFF there is reuse or consumed > 1 time
          for(auto &st:g.members){
              ...
              if(st.func.name()==g.output.func.name()) continue;
              if(g.inlined.find(st.func.name())!=g.inlined.end())  continue;
              //find its consumers - set storage 1 level above reuse (if any)
              for(const auto cons:g.members){
              if(cons.re.size()==0)  continue;
              if(cons.func.name()==st.func.name())  continue;
	      if(g.inlined.find(cons.func.name())!=g.inlined.end())	continue;
              const auto &overlap_dim=cons.re.find(st.func.name());
              if(overlap_dim==cons.re.end())  continue;
              for(const auto &overlap_n:overlap_dim->second){
                    if((!overlap_n.second.defined())||can_prove(overlap_n.second==0))    continue;
		    ...
		    if(st.compute_level.defined()){
			    ...
			    if(stage_pos<stage_old)	new_clevel=false;
		   	}
		    if(new_clevel){
                    st.compute_stage=cons.func.name();
                    st.compute_level=overlap_n.first;
		    cout<<"Stage "<<st.func.name()<<" compute at "<<cons.func.name()<<" , "<<overlap_n.first<<endl;
		    }
              }
            }
        }
     //now we want to make sure that stages with no reuse on the output just move to their externalmost (not a word!) consumers
     //i.e.
        
    for(auto &st:g.members)  cout<<"Mem "<<st.func.name()<<" compute at "<<st.compute_stage<<" , "<<st.compute_level<<endl;
return g;  
...
void Partitioner::initialize_groups() {
    for (pair<const FStage, Group> &g : groups) {
        ...
        pair<map<string, Expr>, GroupAnalysis> best = find_best_tile_config(g.second,true,false);
        g.second.tile_sizes = best.first;
        group_costs.emplace(g.second.output, best.second);
        if(!best.second.cost.memory.defined())  best.second.cost.memory=Float(32).max();
        if(!best.second.cost.arith.defined())  best.second.cost.arith=Float(32).max();

     //   cout<<"g.output "<<g.second.output<<" cost "<<best.second.cost.memory<<endl;
     for(auto &st:g.second.members){
        const auto &st_re=reuse_per_stage.find(st);
        if(st_re!=reuse_per_stage.end())       st.re=st_re->second;
      }     
    }
...
void Partitioner::evaluate_new_tiles() {
 ...
 //  if(g.second.tile_sizes.size()!=0)  continue;
    pair<map<string, Expr>, GroupAnalysis> best = find_best_tile_config(g.second,false,false);
    g.second.tile_sizes = best.first;
    group_costs.emplace(g.second.output, best.second);
}
grouping_cache.clear();
...
void Partitioner::evaluate_final_tiles() {
  ...
  for (pair<const FStage, Group> &g : groups) {
   // if(g.second.members.size()-g.second.inlined.size()>1)  continue;
 //  if(g.second.tile_sizes.size()!=0)  continue;
    pair<map<string, Expr>, GroupAnalysis> best = find_best_tile_config(g.second,false,true);
    g.second.tile_sizes = best.first;
    group_costs.emplace(g.second.output, best.second);
}
grouping_cache.clear();
...
Partitioner::choose_candidate_grouping(const vector<pair<string, string>> &cands,
    ...
    for (const auto &p : cands) {
        ...
        for (const FStage &c : get_element(children, prod)) {
            ...
            if (iter != grouping_cache.end()) {
                ...
                const Function &prod_f = get_element(dep_analysis.env, cand_choice.prod);
              ...
              //      group=inline_rest(group);
               ...
               //     for(const auto &is:group.inlined)   cout<<"new inl after "<<is<<endl;
                }
  ...
  //          internal_assert(best_config.analysis.n_threads.defined());
    ...
    if(!illegal_config) overall_benefit = estimate_benefit(grouping, no_redundant_work, true,level);
    ...
    if (!illegal_config&&overall_benefit.defined() && can_prove(best_benefit <= overall_benefit)) {
...
*/
  ...
  vector<map<string, Expr>> Partitioner::generate_tile_configs(const FStage &stg, bool final_tile) {
    ...
    //cout<<"F_load first "<<stg_name<<" "<<bytes_per_ele<<" bytes"<<endl;
    Expr max_tile=64/bytes_per_ele;


//  map<string,Expr> min_tiles=find_min_tile_dims(stg);
  const vector<Dim> &dims = get_stage_dims(stg.func, stg.stage_num);
...
if(thread_vars.size()>1)   size_variants=size_variants1;
...
if(tile_vars.size()>1){   
           // int* a = nullptr;   // Pointer to int, initialize to nothing.
            ...
            std::vector<int> a(n);
            for (int i=0; i<n; i++) {
        if(thread_vars.find(tile_vars[i])!=thread_vars.end()&&can_prove(extents[tile_vars[i]]>64))  tiles[i]=8;
                  //else if(thread_vars.find(tile_vars[i])!=thread_vars.end()&&can_prove(extents[tile_vars[i]]<=64))  tiles[i]=max(min(extents[tile_vars[i]]/32,2),1);
                  else if(thread_vars.find(tile_vars[i])!=thread_vars.end()&&can_prove(extents[tile_vars[i]]<=64))  tiles[i]=2;
                else if(thread_vars.size()>=2)  tiles[i]=extents[tile_vars[i]];
                else tiles[i]=make_one(Int(32));
      ...
      //  cout<<"GEN TILES "<<tile_vars[index]<<" "<<tiles[index]<<endl;
        if(final_tile&&thread_vars.find(tile_vars[index])!=thread_vars.end()) tiles[index]=simplify(tiles[index]+2);
        else if((!final_tile)&&thread_vars.find(tile_vars[index])!=thread_vars.end()) tiles[index]=simplify(tiles[index]*2);
        else if(thread_vars.find(tile_vars[index])==thread_vars.end()) tiles[index]=extents[tile_vars[index]];
        if(thread_vars.size()<=2&&(thread_vars.find(tile_vars[index])!=thread_vars.end()&&(can_prove(tiles[index]>2*max_tile))))  a[index]=max_int;
        else if (thread_vars.size()>2&&(thread_vars.find(tile_vars[index])!=thread_vars.end()&&(can_prove(tiles[index]>max_tile))))  a[index]=max_int;
        else if ((thread_vars.find(tile_vars[index])!=thread_vars.end()&&(can_prove(extents[tile_vars[index]]/tiles[index]<=2))))  a[index]=max_int;
        if(can_prove(extents[tile_vars[0]]>1024)&&(can_prove(tiles[index]>=extents[tile_vars[0]]/16))) a[index]=max_int;;
        if(can_prove(extents[tile_vars[0]]<1024)&&(can_prove(tiles[index]>extents[tile_vars[0]]/2))) a[index]=max_int;;
        ...
        if(thread_vars.find(tile_vars[index])!=thread_vars.end()&&can_prove(extents[tile_vars[index]]>64))  tiles[index]=8;
        //else if(thread_vars.find(tile_vars[index])!=thread_vars.end()&&can_prove(extents[tile_vars[index]]<=64))  tiles[index]=max(min(extents[tile_vars[index]]/32,2),1);
        else if(thread_vars.find(tile_vars[index])!=thread_vars.end()&&can_prove(extents[tile_vars[index]]<=64))  tiles[index]=2;
        else if(thread_vars.size()>=2 ) tiles[index]=extents[tile_vars[index]];
        else tiles[index]=make_one(Int(32));
        a[index] = 0;
        index++;
 ...
 //should be tile_vars[0]/arch_params.par > 64
 if(stg.stage_num>0){
  if(can_prove(extents[tile_vars[0]]>1024)&&(can_prove(dim_size>=extents[tile_vars[0]]/128))) continue;
  if(can_prove(extents[tile_vars[0]]<1024)&&(can_prove(dim_size>extents[tile_vars[0]]/8))) continue;
  }
  map<string, Expr> tiling;
...
for(const auto &ex:extents){
  ...
  if(tile_vars.find(ex.first)!=tile_vars.end())  continue;
  if(can_prove(ex.second>16))  tile_vars.insert(ex.first);
}
...
for(const auto &tt:tile_vars)  tile_vars_set.emplace(tt);
...
vector<Expr> Partitioner::estimate_occupancy(Expr threads, Expr shared_mem, Expr n_blocks){
  bool debug_flag=false;
          //  Expr threads=eval_max_threads(g,false);
  if(debug_flag)  std::cout<<"Estimating occupancy..."<<std::endl;
  if(debug_flag) std::cout<<"Threads "<<simplify(threads)<<" memory "<<shared_mem<<std::endl;
  /*const Expr &max_regs_per_thread=make_const(Float(64),64);
  const Expr &total_regs_per_SM=make_const(Float(64),65536);
  const Expr &max_regs_per_block=make_const(Float(64),65536);
  const Expr &limit_threads_per_warp=make_const(Float(64),32);
  ...
  Expr num_regs=simplify(cast<int>(min(gparams.max_regs_per_thread, gparams.total_regs_per_SM/threads)));
  if(can_prove(num_regs<1))  num_regs=1;
  else if(can_prove(num_regs>64))  num_regs=make_const(Float(32),64);
  if(debug_flag) std::cout<<"Estimated regs..."<<num_regs<<std::endl;
            //get the number of warps per block
  Expr warps_per_block=simplify(roundUp(threads/gparams.limit_threads_per_warp,1));
  //if(can_prove(warps_per_block<1))  warps_per_block=1;
            //shmem in bytes
  Expr shared_mem_bytes=simplify((max(shared_mem,gparams.min_shared_mem_unit)));


            //regs per block
            // CEILING(CEILING(MyWarpsPerBlock,myWarpAllocationGranularity)*MyRegCount*limitThreadsPerWarp,myAllocationSize), MyWarpsPerBlock)
            //Expr regs_per_block=simplify(max(max(warps_per_block,warp_alloc_granularity)*num_regs*limit_threads_per_warp,reg_alloc_unit_size));
  Expr regs_per_block=warps_per_block;
  if(debug_flag)  std::cout<<"Estimating regs per block..."<<regs_per_block<<std::endl;
            //group regs_per_SM
            //FLOOR(limitRegsPerBlock/CEILING(MyRegCount*limitThreadsPerWarp,myAllocationSize),myWarpAllocationGranularity)
  Expr group_limit_regs_per_SM=roundDown(gparams.max_regs_per_block/roundUp(num_regs*gparams.limit_threads_per_warp,gparams.reg_alloc_unit_size),gparams.warp_alloc_granularity);
  //Expr group_limit_regs_per_SM=simplify((max_regs_per_block/(num_regs*limit_threads_per_warp)));

  if(debug_flag) std::cout<<"Estimating limit regs per SM..."<<regs_per_block<<std::endl;
  ...
  Expr block_group_warps_per_SM=simplify(min(gparams.max_blocks_per_SM,roundDown(gparams.limit_warps_per_SM/warps_per_block,1)));
  //Expr block_group_warps_per_SM=simplify((min(max_blocks_per_SM,(limit_warps_per_SM/warps_per_block))));
  if(debug_flag) std::cout<<"Estimating occupancy limit warps per SM..."<<block_group_warps_per_SM<<std::endl;
            //=IF(MyRegCount>limitRegsPerThread,0,IF(MyRegCount>0,FLOOR(C42/MyRegsPerBlock, 1)*FLOOR(limitTotalRegisters/limitRegsPerBlock,1),limitBlocksPerMultiprocessor))
            //FLOOR(C42/MyRegsPerBlock, 1)*FLOOR(limitTotalRegisters/limitRegsPerBlock,1)
  Expr block_group_regs_per_SM=simplify(roundDown(group_limit_regs_per_SM/regs_per_block,1)*roundDown(gparams.total_regs_per_SM/gparams.max_regs_per_block,1));
  //Expr block_group_regs_per_SM=simplify((group_limit_regs_per_SM/regs_per_block)*(total_regs_per_SM/max_regs_per_block));
  if(debug_flag) std::cout<<"Estimating occupancy limit regs per SM..."<<block_group_regs_per_SM<<std::endl;
            //FLOOR(limitTotalSharedMemory/MySharedMemPerBlock,1),limitBlocksPerMultiprocessor)
  //Expr block_group_shared_mem_per_SM=simplify((limit_shared_mem_per_SM/shared_mem_bytes));
  Expr block_group_shared_mem_per_SM=simplify(roundDown(gparams.limit_shared_mem_per_SM/shared_mem_bytes,1));
  if(debug_flag) std::cout<<"Estimating occupancy limit SH mem per SM..."<<block_group_shared_mem_per_SM<<std::endl;
            //find active blocks per sm
  Expr active_blocks_per_SM=make_one(Int(32));
  if(can_prove(block_group_warps_per_SM<=block_group_regs_per_SM)&&can_prove(block_group_warps_per_SM<=block_group_shared_mem_per_SM)){
    if(debug_flag) std::cout<<"Limited by Max Warps or Max Blocks per Multiprocessor"<<std::endl;
    active_blocks_per_SM=block_group_warps_per_SM;
}else if(can_prove(block_group_regs_per_SM<=block_group_warps_per_SM)&&can_prove(block_group_regs_per_SM<=block_group_shared_mem_per_SM)){
    if(debug_flag) std::cout<<"Limited by Registers per Multiprocessor"<<std::endl;
    active_blocks_per_SM=block_group_regs_per_SM;
}else{
    if(debug_flag) std::cout<<"Limited by Shared Memory per Multiprocessor"<<std::endl;
...
if(debug_flag) std::cout<<"Estimating active warps..."<<active_warps_per_SM<<std::endl;
            //occupancy
Expr occupancy=simplify(active_warps_per_SM/gparams.limit_warps_per_SM);
Expr active_threads=simplify(min(active_warps_per_SM*min(threads,gparams.limit_threads_per_warp),gparams.limit_threads_per_SM));    
Expr active_blocks=active_blocks_per_SM;

if(debug_flag) cout<<"active threads "<<simplify(active_threads)<<endl;
active_blocks=min(n_blocks/active_blocks_per_SM,active_blocks_per_SM);
if(debug_flag) cout<<"active blocks "<<simplify(active_blocks)<<endl;
Expr active_SMs=min(gparams.n_SM,active_blocks*gparams.n_SM); 

 //active_threads=simplify(active_threads*active_SMs);  
if(debug_flag)  cout<<"active SMs "<<simplify(active_SMs)<<endl;
if(debug_flag) std::cout<<" occupancy "<<simplify(occupancy)<<std::endl<<std::endl<<std::endl;
...
Partitioner::find_best_tile_config(const Group &g,bool is_init, bool is_final) {
    ...
    for(const auto &ex:out_extents){
      if(can_prove(ex.second/32>=8))  small_extents=false;
    } 
  //  if(small_extents) cout<<"small extents for "<<g.output<<endl;
  //  if(all_rvars)  cout<<"all rvars in "<<g.output<<endl;
    map<string, Expr> no_tile_config;
    GroupAnalysis best_analysis;
    if(all_rvars)  return make_pair(no_tile_config,best_analysis);
    Group no_tile = g;
    Expr best_n_threads;
    map<string, Expr> best_config;
    Expr best_occupancy;
    
  //  cout<<"find best for "<<g.output.func.name()<<" dim size "<<dims.size()<<endl;
    if(dims.size()==1) return make_pair(no_tile_config,best_analysis);
  ...
  ///  cout<<"gen tiles before"<<endl;
    ...
    else configs=cached_config->second;
    ...
    //  if(config.size()==1)  small_extents=true;
     ...
     //p.tiles=config;
      ...
      if(!is_init)  new_analysis    = analyze_group(new_group, flag_db,false);
      else new_analysis     = analyze_group(new_group, flag_db,true);
      //std::cout<<"BEFORE BENEFIT"<<std::endl;
      if(!new_analysis.defined()) {

       continue;
   }
   if((!test_it)&&(!is_init)&&(!small_extents)&&can_prove(new_analysis.threads_out>=16)){
  ...
  Expr benefit;
  if(!is_final){
  if(!is_init) benefit = estimate_tile_benefit(best_analysis, new_analysis,      false, true);
  else benefit=estimate_benefit(best_analysis, new_analysis,      false, true);
  }else{
...
if(0){
    std::cout<<"TILES for "<<g.output.func.name()<<std::endl<<std::endl;
    for(auto &iti:best_config){
      std::cout<<iti.first<<" "<<iti.second<<std::endl;
  }
  std::cout<<"threads out: "<<best_analysis.threads_out<<" Threads "<<best_n_threads<<std::endl;
...
Expr Partitioner::estimate_threads_out(const Group &g, bool show_analysis){
  ...
  if(flag) std::cout<<"output of group "<<g.output.func.name()<<std::endl;
  ...
  for(const auto &bs:stg_estimates_out){
    ...
    Expr extent = get_extent(bound);
    internal_assert(extent.defined());
    if(flag) std::cout<<"var "<<bs.first<<" "<<bs.second<<std::endl;
    auto dim_in_tiles = g.tile_sizes.find(bs.first);
    if((dim_in_tiles!=g.tile_sizes.end()&&(can_prove(extent>4)))){

      if(can_prove(extent>=64))  spawned_threads=simplify(spawned_threads*bs.second);
  }
}
...
return spawned_threads;
    ...
    pair<map<string,Expr>,vector<pair<FStage,Expr>>> Partitioner::eval_max_threads(const Group &g, bool show_analysis){
    set<string> gmembers;
    for(const auto &st:g.members) gmembers.insert(st.func.name());
          // Get the regions of the pipeline required to compute a tile of the group
     ...
     //   cout<<"VEC "<<vec_var<<endl;
    	...
    	for(const auto &bs:stg_estimates_out){
    		...
    		Expr extent = get_extent(bound);
    		internal_assert(extent.defined());
    		if(flag) std::cout<<"var "<<bs.first<<" "<<bs.second<<"extent "<<extent<<std::endl;
    		auto dim_in_tiles = thread_vars.find(bs.first);
         
     //    if(is_singleton&&dim_in_tiles!=thread_vars.end()&&vec_var.defined()&&(bs.first==vec)){
       ...
       //    continue;
        //   }
    		if((dim_in_tiles!=thread_vars.end()&&(can_prove(extent>0)))){
    			...
    			if(can_prove(extent/bs.second>=2)){
    				if(itiles==max_threads.end())  max_threads[bs.first]=bs.second;
    				else max_threads[bs.first]=max(max_threads[bs.first],bs.second);
    	...
    	for(const auto &stg:local_bounds){
          if(flag)    std::cout<<"mem stg "<<stg.first<<std::endl;
    		auto is_inlined=g.inlined.find(stg.first.func.name());
       if(gmembers.find(stg.first.func.name())==gmembers.end())  continue;
        ...
        //            if(is_singleton&&dim_in_tiles!=thread_vars.end()&&vec_var.defined()&&(iti.first==vec)){
       ...
       //    }
    				if((dim_in_tiles!=thread_vars.end()&&(can_prove(extent>0)))){
    					if(can_prove(extent/iti.second>=2)){
    						...
    						if(itiles==max_threads.end())  max_threads[iti.first]=iti.second;
    						else max_threads[iti.first]=max(max_threads[iti.first],iti.second);
    						if(flag)    std::cout<<stg.first<<" "<<iti.first<<" "<<iti.second<<" extent "<<extent<<std::endl;
    						spawned_threads_local*=iti.second;
    					}
    					else{
    						auto itiles=max_threads.find(iti.first);
    						if(itiles==max_threads.end())  max_threads[iti.first]=extent;
    						else max_threads[iti.first]=max(max_threads[iti.first],extent);
...
Partitioner::GroupAnalysis Partitioner::analyze_group(const Group &g, bool show_analysis,bool to_inline) {
    ...
    for (const auto &stg : g.members) {
      ...
      if(k!=g.inlined.end())  group_inlines.insert(stg.func.name());
...
//cout<<"Group out "<<g.output.func.name()<<" st num "<<g.output.stage_num<<endl;
         // Count the number of tiles

set<string> thread_dims=dims_to_tile(g.output);
bool out_is_update=g.output.stage_num>0;
...
if (!g.output.func.has_extern_definition()) {
  ...
  Definition def = get_stage_definition(g.output.func, g.output.stage_num);
          //    const auto &it=def.values();
         //     for(const auto &it:def.values())  std::cout<<it<<std::endl;
  const vector<Dim> &dims = def.schedule().dims();
  //find first pure var
  int col_it=0;
  for (int d = 0; d < (int)dims.size() - 1; d++) {
  ...
  for (int d = 0; d < (int)dims.size() - 1; d++) {
    ...
    if(d==col_it){
    col_tile=size;  
  //  cout<<"col VAR "<<var<<" size "<<size<<endl;
    }
  //  internal_assert(col_tile.defined());
    Expr dim_tiles = simplify((extent + size - 1) / size);
    if(thread_dims.find(var)!=thread_dims.end()) estimate_blocks*=dim_tiles;
...
//find the inner extents (weak estimate) of each stage (TODO: divide by stride maybe?)
...
for(const auto &dd:group_bounds){
 ...
 const vector<Dim> &dimsf = def.schedule().dims();
  ...
  for(const auto &sgs:stg_estimates){
   ...
   // cout<<"sgs. "<<sgs.first <<" sgs s "<<sgs.second<<endl;
...
std::string param_shared = Internal::get_env_variable("HL_GPU_SHARED_COST");
    ...
    float cost_factor_inline=std::atof(param_inline.c_str());
    float cost_factor_shared= std::atof(param_shared.c_str());
    /*cout<<"merge "<<cost_factor_merge<<endl;
    cout<<"inl "<<cost_factor_inline<<endl;
    cout<<"shared "<<cost_factor_shared<<endl;*/
           //   float load_slope = (arch_params.balance) / arch_params.last_level_cache_size;
//for(const auto &cd:col_dims)  cout<<cd.first<<endl;
for (const auto &f_load : group_load_costs) {
   ...
   // cout<<"F_load first "<<f_load.first<<" "<<bytes_per_ele<<" bytes"<<endl;
    Expr max_tile=64/bytes_per_ele;
  ...
  //  cout<<f_load.first<<std::endl;
    ...
    if (!is_output && is_group_member) {
              //    if(p.pipe[f_load.first][0].is_inline)  continue;
   ...
   //   cout<<"member "<<f_load.first<<endl;
   //   cout<<"loads "<<f_load.second<<endl;
   //   cout<<"footprint "<<footprint<<endl;
   //   cout<<"============"<<endl;
      float cost_factor=cost_factor_shared;
      //toodoo active_threads not always from out    
      if(!to_inline){
   
      
              Expr col_dim;
      if(col_dims.find(f_load.first)==col_dims.end())  col_dim=col_tile;
                    else col_dim=col_dims[f_load.first];
                    col_dim=min(col_dim,col_tile);
                    
      load_slope = (cost_factor)/(48*1024) ;
      //partial_factor+=load_cost*min(1+footprint*load_slope/gparams.limit_shared_mem_per_block,cost_factor);;;
       partial_factor+=load_cost*min(1+footprint*load_slope,cost_factor);;
      }
      else{
      load_slope = (cost_factor)/(1024) ;
      partial_factor+=load_cost*min(1+footprint*load_slope,cost_factor);;;
      }
      ...
      bool is_function = (dep_analysis.env.find(f_load.first) != dep_analysis.env.end());
                    if (!is_function) { // It is a load to some input buffer
                      //SCALE THE LOADS // anything with a footprint larger than the shared mem should get a larger penalty than things that can stay in registers
                      //EVEN MORE FOR INLINING!!!
                      ...
                      float cost_factor;
               if(to_inline){
                      //if(can_prove(footprint>256*1024))  cost_factor_inline;
                      cost_factor=cost_factor_inline;
                     //else if(can_prove(footprint>64)) cost_factor=16;
                     //else cost_factor=arch_params.balance*4;
                     }
                                          else{
           ...
           //           cout<<"loads "<<f_load.second<<endl;
           //           cout<<"footprint "<<footprint<<endl;
           //           cout<<"init footprint "<<initial_footprint<<endl;
           //           cout<<"============="<<endl;
                      load_slope = (cost_factor) /(64*1024);
                  //    partial_factor+=f_load.second*min(1+initial_footprint*load_slope,cost_factor);
                if((!to_inline)) {
                    
                    partial_factor+=footprint*min(1+initial_footprint*load_slope,cost_factor)/min(col_tile,max_tile);
                    partial_factor+=f_load.second*min(1+footprint*load_slope,cost_factor)/min(col_tile,max_tile);
                    }
                else  partial_factor+=f_load.second*min(1+initial_footprint*load_slope,cost_factor);

                 //     partial_factor+=footprint*min(1+initial_footprint*load_slope,cost_factor);;
             ...
             //         cout<<"output "<<f_load.first<<" ";
             //         cout<<"loads "<<f_load.second<<" ";
             //         cout<<"footprint "<<footprint<<endl;
                      float cost_factor;
               if(to_inline){
                      if(can_prove(footprint>256*1024))  cost_factor=cost_factor_inline;
                      
                     //else if(can_prove(footprint>64)) cost_factor=16;
                     else cost_factor=cost_factor_inline;
                     }
                                          else{
                     cost_factor=cost_factor_merge;
  ...
  partial_factor+=footprint*min(1+initial_footprint*load_slope,cost_factor);;
                      partial_factor+=f_load.second*min(1+footprint*load_slope,cost_factor);;
              ...
              //       cout<<"non member "<<f_load.first<<endl;
              //       cout<<"loads "<<f_load.second<<endl;
              //       cout<<"footprint "<<footprint<<endl;      
              //       cout<<"init footprint "<<initial_footprint<<endl;
              //       cout<<"============="<<endl;
                  //else if(can_prove(initial_footprint>1024))   cost_factor=arithch_params.balance/2;
       //           else if(can_prove(initial_footprint>32))   cost_factor=arch_params.balance/4;
        //          else cost_factor=make_const(Int(64),1);cd
            ...
            //         partial_factor+=footprint*min(1+initial_footprint*load_slope,cost_factor);;
                    ...
                    partial_factor+=footprint*min(1+initial_footprint*load_slope,cost_factor)/col_dim;;
                     partial_factor+=f_load.second*min(1+footprint*load_slope,cost_factor)/col_dim;;
                    }
                else  partial_factor+=f_load.second*min(1+initial_footprint*load_slope,cost_factor);
        ...
        GroupAnalysis g_analysis(
            ...
            parallelism);
         //   cout<<"part mem "<<partial_factor<<" n tiles "<<estimate_tiles<<" total "<<simplify(g_analysis.cost.memory)<<endl;
      if(!to_inline){
            	pair<map<string,Expr>,vector<pair<FStage,Expr>>> threads_estimates=eval_max_threads(g,show_analysis);
            	g_analysis.thread_blocks=threads_estimates.first;
              //  cout<<"g an "<<thread_blocks;
            	g_analysis.n_threads=estimate_threads(g_analysis.thread_blocks);
        //    	cout<<"g an "<<out_allocation<<endl;
            	g_analysis.allocated_root=out_allocation;
            	//g_analysis.threads=threads_estimates.second;
            	vector<pair<FStage,Expr>> mem_threads=threads_estimates.second;
            	//g_analysis.threads_out=g_analysis.threads[g.output];
            	//cout<<"g_an "<<g_analysis.threads_out<<" map "<<g_analysis.threads[g.output]<<endl;
            	//internal_assert(can_prove(g_analysis.threads_out==g_analysis.threads[g.output]));
              //  cout<<"g an "<< g_analysis.threads_out<<endl;

            	

  /*          vector<Expr> gpu_specs=estimate_occupancy(g_analysis.threads_out,shared_mem,estimate_blocks);*/
       ...
       // cout<<"TILES"<<endl;
       // for(const auto &tt:g.tile_sizes)  cout<<tt.first<<" "<<tt.second<<endl;
            	const Expr &base_occupancy= make_const(Float(32),0.15);
            	Expr partial_factor=make_zero(Float(64));
            	for ( auto &mem : g.members) {
            		string f_name=mem.func.name();
           // 		cout<<" g costs "<<f_name<<std::endl;
            		if(g.inlined.find(f_name)!=g.inlined.end())	continue;
            		  ////cout<<"here will crash"<<std::endl;
            		bool is_output=(f_name==g.output.func.name()&&mem.stage_num==g.output.stage_num);
            		Cost stage_cost;
            			//get its compute cost
            		if(is_output)	stage_cost=out_cost;
            			else stage_cost=costs.stage_region_cost(f_name,mem.stage_num,compute_regions[mem.func.name()],g.inlined);
            			//get its threads
            			Expr est_mem_threads;
            			for(const auto &thr:mem_threads){
            				if((thr.first.func.name()==f_name)&&(thr.first.stage_num==mem.stage_num)){
            					est_mem_threads=thr.second;
            					if(is_output)	g_analysis.threads_out=thr.second;
            					break;
            				}
            			}
            			//now get its gpu stats
                  //cout<<"getting gpu stats for member "<<f_name<<endl;
            			vector<Expr> gpu_specs=estimate_occupancy(est_mem_threads,shared_mem,estimate_blocks);
            			Expr mem_occupancy=gpu_specs[0];
 //           			if(can_prove(mem_occupancy<base_occupancy))	return GroupAnalysis();
            			Expr mem_active_threads=gpu_specs[1];
            			Expr mem_parallelism=gpu_specs[2];
                  Expr nregs=gpu_specs[3];
                 // cout<<"n regs "<<nregs<<endl;
                  if(can_prove(nregs<64))  return GroupAnalysis();
 							    //cout<<"ost arith "<<stage_cost.arith<<" act threads "<<mem_active_threads<<" occ "<<mem_occupancy<<endl;
            			partial_factor+=stage_cost.arith/(mem_occupancy*mem_active_threads);
                  //cout<<"part factor "<<simplify(partial_factor)<<endl;
       ...
       //     g_analysis.cost.memory=g_analysis.cost.memory/(occ*act_thr);
        ...
        if(!g_analysis.n_threads.defined())  return GroupAnalysis();
        g_analysis.shared_mem=shared_mem;
   /*     if(!to_inline){        	if(can_prove(g_analysis.shared_mem>49152))  return GroupAnalysis();
        	else if(can_prove(g_analysis.n_threads>1024))    return GroupAnalysis();
        	else if(can_prove(g_analysis.allocated_root>2147483647)){
        //		cout<<"THIS SCHED WILL FALLALAL"<<endl;
       // 		cout<<"g.output "<<g.output<<endl;
      ...
      //   return GroupAnalysis();
        ...
        g_analysis.simplify();
       // cout<<"total mem "<<g_analysis.shared_mem<<endl;
       // cout<<"est tiles "<<estimate_tiles<<endl;
       // cout<<"active blocks "<<g_analysis.parallelism<<endl;
  ...
  //  child_group=inline_rest(child_group);
    ...
    group_costs[child] = eval.analysis;
      /*      pair<map<string, Expr>, GroupAnalysis> config = find_best_tile_config(child_group);
          child_group.tile_sizes=config.first;
        //best_tile_config = config.first;
        group_costs[child] = config.second;*/

}

Partitioner::GroupConfig Partitioner::evaluate_choice( Group &group,
  Partitioner::Level level) {
    ...
    if (level == Partitioner::Level::Inline) {
        ...
        map<string, Expr> tile_sizes;
//	for(const auto &mm:group.members){
//	if(mm.func.name().find("output")!=std::string::npos)	return GroupConfig(best_tile_config,group_analysis);
//}
     ...
     //   cout<<"after dims"<<endl;
       ...
       // cout<<"before analyze_group"<<endl;
        group_analysis = analyze_group(group, false,true);
       // cout<<"after analyze_group"<<endl;
     ...
     //   Group 
        ...
        cout<<"==========="<<endl;
        cout<<"group of func "<<group.output.func.name()<<"has boundary stages"<<endl;
        cout<<"members :"<<endl;
        for(const auto &mm:group.members)  cout<<mm.func.name()<<endl;
        cout<<"==========="<<endl;
        ...
        pair<map<string, Expr>, GroupAnalysis> config = find_best_tile_config(group,false,false);
   //     internal_assert(config.second.n_threads.defined());
        best_tile_config = config.first;
...
Expr Partitioner::estimate_tile_benefit(const GroupAnalysis &old_grouping,
 ...
 bool ensure_parallelism
        ...
        //  const Expr &base_occupancy= make_const(Float(32),0.25);
        //  if(!can_prove(new_grouping.occupancy>=base_occupancy))    return Expr();
              if (ensure_parallelism &&
                (!new_grouping.parallelism.defined() ||
                 !can_prove(new_grouping.parallelism >= arch_params.parallelism))) {
               //  cout<<"NO parallelism"<<endl;
                return Expr();
            }  
  if (ensure_parallelism &&
                (!new_grouping.threads_out.defined() ||
                 !can_prove(new_grouping.threads_out >= 16))) {
                // cout<<"NO parallelism threads out"<<endl;
                return Expr();
            }  
  ...
  if(can_prove(new_grouping.shared_mem>gparams.limit_shared_mem_per_block))  return Expr();
  if(can_prove(new_grouping.n_threads>gparams.limit_threads_per_block))    return Expr();
  //cout<<"new group threads"<<new_grouping.n_threads<<" mod "<<simplify(new_grouping.n_threads%32)<<endl;
 if(final_tiles&&can_prove(new_grouping.n_threads%32!=0))  return Expr();
  ...
  if(final_tiles){
    ...
    mem_benefit = old_grouping.cost.memory/(old_grouping.active_threads*old_grouping.occupancy) - new_grouping.cost.memory/(new_grouping.active_threads*new_grouping.occupancy);
   }
  else{
 ...
 /* cout<<"ANALYSIS RESULTS"<<endl;
  ...
  //benefit=(old_grouping.cost.memory+old_grouping.cost.arith)/(old_grouping.active_threads*old_grouping.occupancy)-(new_grouping.cost.memory+new_grouping.cost.arith)/(new_grouping.active_threads*new_grouping.occupancy);
 // cout<<"benefit "<<(benefit)<<endl;
 // cout<<"\n==================\n";
//benefit=(old_grouping.cost.memory+old_grouping.cost.arith)/(old_grouping.threads_out*old_grouping.occupancy)-(new_grouping.cost.memory+new_grouping.cost.arith)/(new_grouping.threads_out*old_grouping.occupancy);
  return (benefit);
}
...
Expr Partitioner::estimate_benefit(const GroupAnalysis &old_grouping,
   ...
   bool ensure_parallelism) {
    ...
    /*if (ensure_parallelism &&
        (!new_grouping.parallelism.defined() ||
         !can_prove(new_grouping.parallelism >= 14))) {
        return Expr();
    }*/
...
Expr Partitioner::estimate_benefit(
    ...
    for (const auto &g : new_grouping) {
        ...
        if (analysisg.defined()) {
           // cout<<"analysisgnew"<<endl;
         //   cout<<"thout "<<analysisg.threads_out<<endl;
        //    cout<<"nthr "<<analysisg.n_threads<<endl;
        //    cout<<"occ "<<analysisg.occupancy<<endl;
            new_group_analysis.cost.arith += analysisg.cost.arith;
            new_group_analysis.shared_mem+=new_group_analysis.shared_mem;
            new_group_analysis.threads_out=min(new_group_analysis.threads_out,analysisg.threads_out);
            new_group_analysis.active_threads=min(new_group_analysis.active_threads,analysisg.active_threads);
            new_group_analysis.n_threads=max(new_group_analysis.n_threads,analysisg.n_threads);
            ...
            new_group_analysis.occupancy = min(new_group_analysis.occupancy,
             analysisg.occupancy);
        } else {
        ...
        if (analysisg.defined()) {
           ...
           // cout<<"occ "<<analysisg.occupancy<<endl;
            ...
            old_group_analysis.parallelism = min(old_group_analysis.parallelism,
             analysisg.parallelism);
            old_group_analysis.threads_out = min(old_group_analysis.threads_out,
             analysisg.threads_out);
               old_group_analysis.active_threads = min(old_group_analysis.active_threads,
             analysisg.active_threads);
          
            old_group_analysis.occupancy = min(old_group_analysis.occupancy,
             analysisg.occupancy);
            old_group_analysis.n_threads=max(old_group_analysis.n_threads,analysisg.n_threads);
        } else {
    ...
    if(level==Partitioner::Level::Inline)    return estimate_benefit(old_group_analysis, new_group_analysis,
        no_redundant_work, ensure_parallelism);
        else return estimate_tile_benefit(old_group_analysis, new_group_analysis,
            no_redundant_work, ensure_parallelism);
    }
...
void Partitioner::vectorize_stage(const Group &g, Stage f_handle, int stage_num,
    Definition def, Function func, bool is_group_output, bool is_singleton,
    ...
    const auto &can_thread = find(thread_dims.begin(),thread_dims.end(),dim_name);
    const auto &iter = estimates.find(dim_name);
    if ((iter != estimates.end()) && iter->second.defined()) {
        if (can_vectorize) {
          if(can_thread!=thread_dims.end() && can_prove(iter->second<64) && vec_dim_indices.size()==0){
            flag_lane=true;
            for(int dd=0;dd<d;dd++){
              if(!dims[dd].is_rvar())  flag_lane=false;
            }
            vec_dim_indices.push_back(d);
            n_threads++;
            if(n_threads>=3)  break;
        }
...
void Partitioner::unroll_group_inner_stage(const Group &g, Stage f_handle, int stage_num,
    ...
    for (int d = 0; d < (int) dims.size() - 1; d++) {
      ...
      string dim_name = get_base_name(dims[d].var);
      const auto &can_thread = find(thread_dims.begin(),thread_dims.end(),dim_name);
      const auto &non_blocked = find(inner_non_threads.begin(),inner_non_threads.end(),dim_name);
      const auto &iter = estimates.find(dim_name);
      bool is_bounded=false;
      if ((iter != estimates.end()) && iter->second.defined()) {
        if(!dims[d].is_rvar()){
            for(const auto & bbs:g.output.func.schedule().bounds()){
                if(bbs.var==dim_name) is_bounded=true;
            }
            if(!is_bounded) continue;
        }
        //if(can_thread==thread_dims.end() && non_blocked==inner_non_threads.end() && can_prove(iter->second<=4) && vec_dim_indices.size()==0){
        if(can_thread==thread_dims.end() && non_blocked==inner_non_threads.end() && can_prove(iter->second<=4)){
...
void Partitioner::unroll_group_outer_stage(const Group &g, Stage f_handle, int stage_num,
    Definition def, Function func, bool is_group_output,
    const Target &t, set<string> &rvars,
    map<string, Expr> &estimates, AutoSchedule &sched,
    vector<string> thread_dims,vector<string> inner_non_threads) {
    vector<Dim> &dims = def.schedule().dims();
    FStage stg(func,stage_num);
    DimBounds stg_bounds = get_bounds(stg);
    map<string,Expr> extents= bounds_to_estimates(stg_bounds);
   
   
    vector<VarOrRVar> Un_Dims;
    bool flag_vec=true;
        for (int d = 0; d < (int) dims.size() - 1; d++) {
        if (dims[d].for_type == ForType::Vectorized) {
          flag_vec=false;
    ...
    if(f_handle.name().find("denoised")!=std::string::npos)	flag_vec=false;
    for (int d = 0; d < (int) dims.size() - 1; d++) {
        if(!flag_vec)  break;
      string dim_name = get_base_name(dims[d].var);
      const auto &can_thread = find(thread_dims.begin(),thread_dims.end(),dim_name);
      ...
      if ((iter != estimates.end()) && iter->second.defined()) {
        //if(can_thread==thread_dims.end() && non_blocked==inner_non_threads.end() && can_prove(iter->second<=4) && vec_dim_indices.size()==0){
        if(can_thread!=thread_dims.end()&&can_prove(iter->second>=4)&&can_prove(iter->second%2==0)){
        //if(d<=1&&can_thread!=thread_dims.end()&&can_prove(iter->second>=4)){        
          cout<<"Stage "<<f_handle.name()<<endl;
	
          cout<<"Will unroll outer "<<iter->first<<" iter size "<<iter->second<<" extent "<<extents[iter->first]<<endl;
          
          VarOrRVar vec_var(dims[d].var, dims[d].is_rvar());
          Un_Dims.push_back(vec_var);
          break;
//	if(!dims[d].	
      }
	if(can_thread!=thread_dims.end()&&(!dims[d].is_rvar()))	break;




  }
  if(Un_Dims.size()>=2)  break;
}
if(Un_Dims.size()>0){

    VarOrRVar vec_var=Un_Dims[0];
    sched.push_schedule(f_handle.name(), stage_num,
      "unroll(" + vec_var.name() +", 2,TailStrategy::GuardWithIf)",
      {vec_var.name()});
    f_handle.unroll(vec_var,2,TailStrategy::GuardWithIf);              

}
/*
else if(Un_Dims.size()==2){
vector<VarOrRVar> outer_splits;
vector<VarOrRVar> inner_splits;
for(auto u_var:Un_Dims){
    //if we have 2 threads unrolled we first have to tile them
    string vec_dim_name = get_base_name(u_var.name());
    pair<VarOrRVar, VarOrRVar> split_vars =
    split_dim(g, f_handle, stage_num, def, is_group_output, u_var, 2,
              "_vi", "_vo", estimates, sched);
    inner_splits.push_back(split_vars.first);
    outer_splits.push_back(split_vars.second);                  
...
for(const auto &vars:inner_splits){
  ordering.push_back(vars);
}
for(const auto &vars:outer_splits){
  ordering.push_back(vars);
}
...
void Partitioner::reorder_dims(Stage f_handle, int stage_num, Definition def,
 map<string, Expr> strides, AutoSchedule &sched,map<string,Expr> sbounds,vector<string> threads) {
  vector<Dim> &dims = def.schedule().dims();
...
for(int d=0;d<(int)dims.size()-1;d++){
    ...
    bool is_thread=find(threads.begin(),threads.end(),var_name)!=threads.end();
...
for(int d=0;d<(int)dims.size()-1;d++){
    ...
    if(iter!=sbounds.end()&&(!is_thread)){
      pair<string,int> lord(iter->first,d);
      const auto &already_set=(find(order.begin(),order.end(),lord)!=order.end());
      if(!already_set){  
        order.push_back(lord);
        strides.erase(iter->first);
               // cout<<" order "<<iter->first<<" "<<oo<<endl;
...
void Partitioner::generate_group_cpu_schedule(
  const Group &pre_gr, const Target &t,
  const map<FStage, DimBounds> &group_loop_bounds,
  const map<string, Box> &group_storage_bounds,
  const set<string> &inlines,
  AutoSchedule &sched) {
//   Group g=pre_gr;
  Group g=optimize_granularity(pre_gr,sched);
  
  string out_f_name = g.output.func.name();
  Function g_out = g.output.func;
 
  //cout<<"generating schedule of group "<<out_f_name<<endl;
  //for(const auto &mem:g.members)  cout<<mem.func.name()<<" asd"<<endl;
    if (g.output.func.has_extern_definition()) {
...
/*if((dims.size()==2)||(dims.size()-rvars.size()==2)){
  f_handle.gpu_single_thread();
  sched.push_schedule(f_handle.name(), g.output.stage_num,
    "gpu_single_thread()", {});
  return;
...
//   std::cout<<"in stage "<<f_handle.name()<<" "<<var<<" tile is 1 but extent is "<<get_element(stg_estimates, var)<<std::endl;
 //  cout<<"outer non blocked "<<v.var<<endl;
  ...
  // if(var=="dx")  std::cout<<"in stage "<<f_handle.name()<<" "<<var<<" tile is "<<tile_size<<"but extent is "<<get_element(stg_estimates, var)<<std::endl;
...
if (is_rvar) {
  ...
  //  const auto &iter = g.tile_sizes.find(var);
  //  if ((iter == g.tile_sizes.end())) cout<<"was not tiled"<<endl;
   // else cout<<iter->second<<endl;
  //  std::cout<<"in stage "<<f_handle.name()<<" "<<var<<"  not tiled but extent is "<<get_element(stg_estimates, var)<<std::endl;
          //  bool is_intra=p.intra_tile.find(v.name())!=p.intra_tile.end();
          //  bool is_inter=p.inter_tile.find(v.name())!=p.inter_tile.end();

    if((thread_ests.size()<=1)&&(!v.is_rvar)){
   //     cout<<"non tiled blocked "<<v.var<<endl;
        outer_dims.push_back(v);
        block_dims.push_back(v.name());
    }
    else {
  //      cout<<"inner non threads"<<v.var<<endl;
        ...
        /*cout<<"INTER DIMS "<<endl;
        for(const auto &ip:p.inter_tile){
          cout<<ip<<endl;
        }
        cout<<"INTRA DIMS "<<endl;
        for(const auto &ip:p.intra_tile){
          cout<<ip<<endl;
        }*/
//cout<<"=========="<<endl;
if (!outer_dims.empty()) {
...
bool is_singleton=is_singleton_group(g);
//opcout<<"is singleton"<<g.members.size()<<" output "<<g.output.stage_num<<endl;
vectorize_stage(g, f_handle, g.output.stage_num, def, g_out, true,is_singleton, t,
...
if ((iter != stg_estimates.end()) && iter->second.defined()&&(iter2!=block_dims.end()))  {
  if (seq_var != "") {
...
cout<<"intra tile var "<<intra_tile_var.name();
        //for luts just use gpu_thread;
...
if(mem.stage_num>0)	mem_handle = Func(mem.func).update(mem.stage_num-1);
if (dims.size() > 2) {
  ...
  if (!mem_strides.empty()) {
    ...
    map<string,Expr> sbounds= find_dims(mem,mem.stage_num);
         //   cout<<"after dims"<<endl;
    reorder_dims(mem_handle, mem.stage_num, mem_def, mem_strides, sched,sbounds,thread_dims);
...
string clevel=get_expr_str(mem.compute_level);
Function comp_at;
bool found=false;
for(const auto &memb:g.members){
    ...
    cout<<" mem func "<<mem.func.name()<<endl;       
		comp_at=memb.func;
		if(memb.stage_num!=memb.func.updates().size())	continue;
        ...
        found=true;
			  ...
			  cout<<"found at i "<<i<<" "<<dims[i].var<<endl;
			  if(i+1<(int)dims.size()-1){
			  tile_inner_var=VarOrRVar(dims[i+1].var,dims[i+1].is_rvar());
...
if(!found)  vectorize_stage(g, mem_handle, mem.stage_num, mem_def, mem.func,is_output_st,is_singleton,
  t, mem_rvars, mem_estimates, sched,thread_dims);
        unroll_group_inner_stage(g, mem_handle, mem.stage_num, mem_def, mem.func, false,
         t, mem_rvars, mem_estimates, sched,thread_dims,block_dims);
      //  unroll_group_outer_stage(g, mem_handle, mem.stage_num, mem_def, mem.func, false,
      //   t, mem_rvars, mem_estimates, sched,thread_dims,block_dims);
   ...
   /* cout<<"================================="<<endl;
    ...
    for(const auto &db:loop_bounds){
        ...
        for(const auto &mb:db.second){
            ...
            for(const auto &es:ests) cout<<"var "<<es.first <<" est "<<es.second<<endl;
        }    
    }
    cout<<"EXTENTS"<<endl;
    for(const auto &db:loop_bounds){
...
MachineParams MachineParams::generic() {
    ...
    if (params.empty()) {
        return MachineParams(14, 16 * 1024 * 1024, 4);
    } else {
