...
int main(int argc, char **argv) {
  if (argc < 7) {
    printf("Usage: ./process input.png slices focus_depth blur_radius_scale "
           "aperture_samples timing_iterations output.png\n"
  ...
  multi_way_bench(
      {{"Manual",
       ...
       {"Nested auto-scheduled",
        [&]() {
          lens_blur_auto_schedule_store(left_im, right_im, slices, focus_depth,
                                        blur_radius_scale, aperture_samples,
                                        output);
          output.device_sync();
        }},
       {"No-fusion auto-scheduled", [&]() {
          lens_blur_auto_schedule_no_fus(left_im, right_im, slices, focus_depth,
                                         blur_radius_scale, aperture_samples,
                                         output);
          output.device_sync();
        }}});
