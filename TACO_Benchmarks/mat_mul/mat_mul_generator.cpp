...
//	matrix_mul(x,y)=cast<float>(0);
	...
	printf("MAN GPU MAAAAN\n");
                    Var xi("xi"), yi("yi"), xii("xii"), yii("yii"), xt("xt"), yt("yt");
        output.tile(x,y,xi,yi,8,8).unroll(xi).unroll(yi).gpu_tile(x, y, xt, yt, 2, 2);
   //     matrix_mul.compute_at(output, xt).update().reorder(x, y, k.x);
	matrix_mul.compute_at(output,x).gpu_threads(x,y).update().gpu_threads(x,y);
/*
        Var t;
       matrix_mul.update()
